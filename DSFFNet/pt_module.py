...
# from DSFFNet.gcn_module import GCNConv, EdgeConv

# from utils import quaternion, transform
...
class PoseFeature(torch.nn.Module):
    ...
    def forward(self, v):  # x: 3,BVp
        ...
        x, _ = torch.max(x, dim=-1, keepdim=True)  # 取dim=2上的最大值 B,1024,V
        return x  # B,1024,1
...
class FFAdaINResBlock(torch.nn.Module):
    def __init__(self, C):
        ...
        # self.conv3 = torch.nn.Conv1d(C, C, 1)
        self.test1 = FFAdaIN(C)
        self.test2 = FFAdaIN(C)
        # self.test3 = TestModule(C)

    def forward(self, x, x_id, x_po):
        x1 = self.test1(x, x_id, x_po)
        x1 = self.conv1(torch.relu(x1))
        x2 = self.test2(x1, x_id, x_po)
        x2 = self.conv2(torch.relu(x2))
        # x3 = self.test3(x, x_id, x_po)
...
class FFAdaINDecoder(torch.nn.Module):
    def __init__(self):
        ...
        self.testres1 = FFAdaINResBlock(256)
        self.conv2 = torch.nn.Conv1d(256, 1024, 1)
        self.testres2 = FFAdaINResBlock(1024)
        self.conv3 = torch.nn.Conv1d(1024, 256, 1)
        self.testres3 = FFAdaINResBlock(256)
        self.conv4 = torch.nn.Conv1d(256, 3, 1)

    def forward(self, x, x_id, x_po):
        x_m = self.conv1(x)
        x_m = self.testres1(x_m, x_id, x_po)
        x_m = self.conv2(x_m)
        x_m = self.testres2(x_m, x_id, x_po)
        x_m = self.conv3(x_m)
        x_m = self.testres3(x_m, x_id, x_po)
        x_m = self.conv4(x_m)
        x_m = 2 * torch.tanh(x_m)
        return x_m

# 使用的模型
class FFAdaINModel(torch.nn.Module):
    def __init__(self):
        super(FFAdaINModel, self).__init__()
